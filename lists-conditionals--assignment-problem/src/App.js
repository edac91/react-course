...
class App extends Component {
    ...
    onTextChanged = (event) => {
        ...
        this.setState({textLength: length, inputValue: text, charArray: text.split('')});
    }

    deleteCharOnText = index => {
        const charArray = [...this.state.charArray];
        charArray.splice(index, 1);
        const text = charArray.join('');
        this.setState({charArray: charArray, inputValue: text, textLength: text.length});
    }
    ...
    render() {
                ...
                <div>
                    {this.state.charArray.map((char, index) => {
                        return <CharComponent
                            click={() => this.deleteCharOnText(index)}
                            text={char}
                            key={char + index}/>
                    })}
                </div>
        ...
        return (
            <div className="App">
                ...
                <hr/>
                <input type="text" onChange={this.onTextChanged} value={this.state.inputValue}/>
